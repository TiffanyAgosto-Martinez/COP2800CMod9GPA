...
public class PenguinAnalyzer {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens.length >= 7) { // Check for 7 columns
                    try { // Read sample number
                        int sampleNumber = Integer.parseInt(tokens[0].trim());
                        String species = tokens[1].trim();
                        double billLength = Double.parseDouble
                                           (tokens[2].trim());
                        double billDepth = Double.parseDouble
                                          (tokens[3].trim());
                        double flipperLength = Double.parseDouble
                                             (tokens[4].trim());
                        double bodyMass = Double.parseDouble(tokens[4].trim());
                        String sex = tokens[6].trim();
                        
                        PenguinData penguin = 
                            new PenguinData(sampleNumber, species, billLength,
    ...
    public void printPenguinData() {
        ...
        for (PenguinData penguin : penguinList) {
            System.out.printf("%-5d %-10s %-10.1f %-10.1f %-15.1f %-10.1f" +       
                   " %-8s%n", penguin.getSampleNumber(), penguin.getSpecies(), 
                   penguin.getBillLength(), penguin.getBillDepth(), 
    ...
    public void writePenguinData() {
        try (PrintWriter writer = new PrintWriter(new FileWriter
            ...
            for (PenguinData penguin : penguinList) {
                writer.printf("%-5d %-10s %-10.1f %-10.1f %-15.1f %-10.1f" +  
                       "%-8s%n", penguin.getSampleNumber(), 
                       penguin.getSpecies(), penguin.getBillLength(),
                       penguin.getBillDepth(), penguin.getFlipperLength(),
                       penguin.getBodyMass(), penguin.getSex());
            }
            System.out.println("Penguin data successfully written to " + 
                              "PenguinOutput.txt.");
        } catch (IOException e) {
